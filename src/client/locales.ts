/** `voice` namespace dictionaries (the composer microphone control and the voice settings card). */

/** Simplified Chinese dictionary (the key-set source of truth). */
export const zh = {
  'mic.idle.aria': '开始语音输入',
  'mic.idle.title': '开始语音输入',
  'mic.hold.title': '按住说话',
  'mic.recording.aria': '正在录音，按下停止并转写',
  'mic.recording.title': '正在录音 — 点击停止并转写',
  'mic.transcribing.aria': '正在转写',
  'mic.transcribing.title': '正在转写…',
  'mic.unconfigured.title': '语音输入尚未配置',
  'mic.polishing.title': '正在润色转写结果…',
  'mic.polishing.short': '润色中…',
  'settings.polish': 'AI 润色',
  'settings.polish.on': '开启（使用部署的默认模型）',
  'settings.polish.off': '关闭（保留原始转写）',
  'settings.polish.hint': '润色会把每条转写文本发送给该模型。即使用本地 whisper.cpp 转写，若该模型是云端服务，音频留在本机，文本仍会发送出去。',
  'settings.polishPrompt': '润色提示词（留空使用内置）',
  'settings.silenceStop': '静音自动停止（毫秒）',
  'settings.silenceStop.hint': '填 0 关闭；留空则恢复部署的默认值。',
  'settings.liveInterval': '实时预览间隔（毫秒）',
  'settings.liveInterval.hint': '录音期间每隔一段时间重新转写已录制内容。本地模型开销很小；云端服务按次计费。填 0 关闭；留空则恢复部署的默认值。',
  'settings.device': '麦克风',
  'settings.device.default': '系统默认',
  'settings.device.unnamed': '未命名麦克风',
  'settings.device.hint': '仅保存在本机，不会同步。',
  'settings.expand': '展开',
  'settings.collapse': '收起',
  'settings.unsaved': '未保存',
  'settings.save': '保存',
  'settings.saving': '保存中…',
  'settings.discard': '放弃',
  'settings.saveFailed': '保存失败，请重试。',
  'settings.readOnly': '此配置由部署方管理，无法在此修改。',
  'settings.invalidNumber': '请输入正整数。',
  'settings.title': '语音输入',
  'settings.description': '用麦克风口述提示词。转写文本会进入输入框，发送前可以修改。',
  'settings.provider': '转写服务',
  'settings.provider.none': '未挂载转写服务',
  'settings.status.ready': '就绪',
  'settings.status.notReady': '未就绪',
  'settings.interaction': '录音方式',
  'settings.interaction.toggle': '点击开始 / 点击结束',
  'settings.interaction.hold': '按住说话',
  'settings.insert': '转写结果',
  'settings.insert.append': '追加到输入框',
  'settings.insert.replace': '替换输入框',
  'settings.maxClipSeconds': '最长录音（秒）',
  'settings.language': '语言（BCP-47，留空自动识别）',
} satisfies Record<string, string>

/** The voice namespace key union. */
export type VoiceKey = keyof typeof zh

/** English dictionary, checked complete against the zh key set. */
export const en = {
  'mic.idle.aria': 'Start voice input',
  'mic.idle.title': 'Start voice input',
  'mic.hold.title': 'Hold to talk',
  'mic.recording.aria': 'Recording, press to stop and transcribe',
  'mic.recording.title': 'Recording — click to stop and transcribe',
  'mic.transcribing.aria': 'Transcribing',
  'mic.transcribing.title': 'Transcribing…',
  'mic.unconfigured.title': 'Voice input is not configured',
  'mic.polishing.title': 'Cleaning up the transcript…',
  'mic.polishing.short': 'polishing…',
  'settings.polish': 'AI polish',
  'settings.polish.on': 'On (uses the deployment\'s default model)',
  'settings.polish.off': 'Off (keep the raw transcript)',
  'settings.polish.hint': 'Polishing sends each transcript to that model. With the local whisper.cpp provider and a hosted model, the audio stays on this machine but the text does not.',
  'settings.polishPrompt': 'Polish prompt (blank uses the built-in one)',
  'settings.silenceStop': 'Stop after silence (ms)',
  'settings.silenceStop.hint': '0 turns it off. Blank restores the deployment default.',
  'settings.liveInterval': 'Live preview interval (ms)',
  'settings.liveInterval.hint': 'Re-transcribes what has been recorded so far while you speak. Cheap against a local model; billed per pass against a hosted one. 0 turns it off; blank restores the deployment default.',
  'settings.device': 'Microphone',
  'settings.device.default': 'System default',
  'settings.device.unnamed': 'Unnamed microphone',
  'settings.device.hint': 'Stored on this machine only; never synced.',
  'settings.expand': 'Expand',
  'settings.collapse': 'Collapse',
  'settings.unsaved': 'Unsaved',
  'settings.save': 'Save',
  'settings.saving': 'Saving…',
  'settings.discard': 'Discard',
  'settings.saveFailed': 'Could not save. Try again.',
  'settings.readOnly': 'This deployment manages these settings; they cannot be changed here.',
  'settings.invalidNumber': 'Enter a whole number greater than zero.',
  'settings.title': 'Voice input',
  'settings.description': 'Dictate prompts with your microphone. The transcript lands in the composer, where you can edit it before sending.',
  'settings.provider': 'Transcription provider',
  'settings.provider.none': 'No transcription provider is mounted',
  'settings.status.ready': 'Ready',
  'settings.status.notReady': 'Not ready',
  'settings.interaction': 'Recording gesture',
  'settings.interaction.toggle': 'Click to start, click to stop',
  'settings.interaction.hold': 'Hold to talk',
  'settings.insert': 'Transcript',
  'settings.insert.append': 'Append to the composer',
  'settings.insert.replace': 'Replace the composer',
  'settings.maxClipSeconds': 'Maximum recording (seconds)',
  'settings.language': 'Language (BCP-47; blank auto-detects)',
} satisfies Record<VoiceKey, string>
